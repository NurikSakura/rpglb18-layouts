...
(function () {
	...
	const CHALLENGE_PIP_INTERVAL = 0.005;
	const TUG_PIP_INTERVAL = 0.00667;
	const CHOICE_BAR_WIDTH = 214;
	const CHOICE_PIP_INTERVAL = 0.0075;
	const BIG_PIP_WIDTH = 1;
	const SMALL_PIP_WIDTH = 1;
	const EMPTY_OBJ = {};
	...
	Polymer({
		...
		showBid(bid, index, bidsArray) {
			...
			const bidDescriptionHeight = bid.type === 'choice-many' ? 75 : 75;
			if (index === 0) {
				this.tl.set(this.$.bidDescription, {
					height: bidDescriptionHeight
				});
			} else {
				const previousBidDescriptionHeight = bidsArray[index - 1].type === 'choice-many' ? 75 : 75;
